...
async function countriesFun(params) {
  const response = await fetch(countriesAPI);
  const countries = await response.json();
  for (const { name, capital, languages, population, area } of countries) {
...
async function catsAPIfun(params) {
  try {
...
// async function langaugesAsOfficials(params) {
//   const response = await fetch(countriesAPI);
//   const countries = await response.json();
